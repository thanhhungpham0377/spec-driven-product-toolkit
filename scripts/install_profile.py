...
def load_registry() -> dict:
    return load_json(ROOT / "components" / "registry.json")["components"]

def load_profile(name: str) -> dict:
    return load_json(ROOT / "profiles" / f"{name}.json")
...
def main() -> int:
    ...
    selected = [name for name, item in registry.items() if item["capability"] in profile["capabilities"]]
    missing = [name for name in selected if not present(registry[name]["command"])]
    kit = target / ".spec-product"
    manifest = {
        "profile": args.profile,
        "schema_version": 1,
        "toolkit_version": (ROOT / "VERSION").read_text(encoding="utf-8").strip(),
        "profile_definition": f"profiles/{args.profile}.json",
        "components": [{"name": n, **registry[n]} for n in selected],
        "capabilities": profile["capabilities"],
        "non_overlap": True,
        "missing_commands": [f"Install or enable: {n} ({registry[n]['source']})" for n in missing],
    }
    ...
    (kit / "source-catalog.md").write_text("# Installed source catalog\n\n" + "\n".join(f"- **{n}** — `{registry[n]['capability']}` — {registry[n]['source']}" for n in selected) + "\n", encoding="utf-8")
    ...
    if missing:
        ...
        for name in missing:
            print(f"- {name}: {registry[name]['source']}")
        return 2
